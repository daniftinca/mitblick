...
class MatIconRegistry {
    ...
    _fetchUrl(safeUrl) {
        ...
        /** @type {?} */
        const req = this._httpClient.get(url, {responseType: 'text'}).pipe(finalize(() = > this._inProgressUrlFetches.delete(url)),
            share
        ()
    )
        ;
...
/** @nocollapse */ MatIconRegistry.ngInjectableDef = defineInjectable({
    ...
    ngOnChanges(changes) {
        ...
        if (changes.svgIcon) {
            if (this.svgIcon) {
                ...
                this._iconRegistry.getNamedSvgIcon(iconName, namespace).pipe(take(1)).subscribe(svg = > this._setSvgElement(svg), (err) =
            >
